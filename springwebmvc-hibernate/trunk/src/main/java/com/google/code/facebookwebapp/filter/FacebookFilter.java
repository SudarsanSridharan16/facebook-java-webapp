...
public class FacebookFilter implements Filter {
	...
	public Map<String, String> getFacebookCookie(HttpServletRequest request) throws NoSuchAlgorithmException, UnsupportedEncodingException {
		...
		// 1. Check that we have the required fields
		if (appId == null ||
		...
		// 3. Get the arguments separately
		String[] args = fbs.split("&");
		
		// 4. Sort the array
		Arrays.sort(args);
		
		// 5. Get payload and sig argument
		String payload = "";
		String sig_arg = null;
		...
		// 8. convert the array to the result map
